...
from model.clip_detris import build_model
from .layers_detris import Neck, Decoder, Projector
from .fusion_detris import Fusion
...
class DETRIS(nn.Module):
    ...
    def forward(self, img, word, mask=None, encoder_feats=False, decoder_feats=False, text_feats=False):
        '''
            img: b, 3, h, w
            word: b, words
            word_mask: b, words
        ...
        # vis: C3 (b, 768, 32, 32) / C4 (b, 768, 32, 32) / C5 (b, 768, 32, 32)
        # word: b, length, 512
        # state: b, 512

        vis, word, state= self.fusion(img, word, self.txt_backbone, self.dinov2)
        txt_feats = state.detach() if text_feats else None
        # enc_feats = vis[-1].detach() if encoder_feats else None
        # enc_feats = word.detach() if encoder_feats else None
        # b, 512, 26, 26 (C4)
        fq = self.neck(vis, state)
        b, c, h, w = fq.size()
        enc_feats = fq.detach() if encoder_feats else None
        fq = self.decoder(fq, word, pad_mask)
        fq = fq.reshape(b, c, h, w) # b, 512, 32, 32
        dec_feats = fq.detach() if decoder_feats else None

        # b, 1, 104, 104
        pred = self.proj(fq, state) # b, 1, 128, 128
        
        if text_feats and encoder_feats:
            return pred.detach(), enc_feats, txt_feats
        if decoder_feats and encoder_feats:
            return pred.detach(), enc_feats, dec_feats
        if decoder_feats:
            return pred.detach(), dec_feats
        if encoder_feats:
            return pred.detach(), enc_feats
            # return pred.detach(), fq.detach()   
