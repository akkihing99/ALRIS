...
from .adapter_detris import TextAdapter
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self,
        ...
        if use_adapter:
            self.adapter= TextAdapter(d_model, txt_adapter_dim, txt_adapter_dim//2, txt_adapter_dim//16,txt_adapter_dim//4,txt_adapter_dim//16,txt_adapter_dim//4) #fc_in_channels, in_channels
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.use_adapter:
            adapter_feature = x.clone()
            # L B D -> B L D
            adapter_feature = adapter_feature.transpose(0, 1)
            adapter_feature = self.adapter(adapter_feature)
            # B L D -> L B D
            adapter_feature = adapter_feature.transpose(0, 1)
            
            x = x + self.mlp(self.ln_2(x))+0.2*adapter_feature
        else:
...
class Transformer(nn.Module):
    def __init__(self,
        ...
        self.width = width
        self.layers = layers
        add_adapter_layer = set(add_adapter_layer)
        use_adapter = []
        for i in range(layers):
            use_adapter.append(i in add_adapter_layer)
        self.resblocks = nn.Sequential(*[
            ResidualAttentionBlock(width, heads, attn_mask, use_adapter[i],txt_adapter_dim)
            for i in range(layers)
        ])
...
def build_model(state_dict: dict, txt_length: int, new_resolution: int = -1, add_adapter_layer = [],txt_adapter_dim=384):
    ...
    model = CLIP(embed_dim, new_resolution, vision_layers, vision_width,
                 ...
                 add_adapter_layer,txt_adapter_dim)
