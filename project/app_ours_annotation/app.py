...
print("[INFO] Loading ViP-LLaVA model...")
...
def load_image_tensor_with_box(img_path, mask_path, image_processor):
    ...
    return image_processor.preprocess(pil, return_tensors='pt')['pixel_values'].half().cuda()
...
def index():
    ...
    # ✨ FIX: Redirect to a template is replaced with a simple message.
...
def text_annotate(image_name):
    ...
    if prompt_ids_key not in session or session.get(current_image_key) != image_name:
        ...
        print(f"[TEXT] Initialized with prefix: '{prefix_text}' -> token_ids={prefix_token_ids} -> decoded='{decoded_prefix}'")
